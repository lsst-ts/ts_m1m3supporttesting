...
class ForceBalanceSystemCheckProfile:
    def Run(self, m1m3, sim, efd):
        ...
        while (data.Timestamp - startTimestamp) < TEST_HOLD_TIME:
            ...
            if rtn >= 0:
                faOutput = faOutput + ("%0.6f,0,691,972,0,%0.3f,%0.3f\r\n" % (faData.Timestamp, faData.YForce[actuatorYIndex], faData.ZForce[actuatorZIndex]))
        ...
        for record in testTable:           
            ...
            # Append data to file
            hpOutput = hpOutput + ("%0.6f,%0.6f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f\r\n" % (data.Timestamp, (data.Timestamp - realStart), 0, 0, 0, 0, 0, 0, data.MeasuredForce[0], data.MeasuredForce[1], data.MeasuredForce[2], data.MeasuredForce[3], data.MeasuredForce[4], data.MeasuredForce[5], data.Fx, data.Fy, data.Fz, data.Mx, data.My, data.Mz))
            ...
            rtn, faData = m1m3.GetNextSampleForceActuatorData()
            if rtn >= 0:
                faOutput = faOutput + ("%0.6f,0,%0.3f,%0.3f,0,%0.3f,%0.3f\r\n" % (faData.Timestamp, record[1], record[2], faData.YForce[actuatorYIndex], faData.ZForce[actuatorZIndex]))
        ...
        while (data.Timestamp - startTimestamp) < TEST_HOLD_TIME:
            ...
            rtn, faData = m1m3.GetNextSampleForceActuatorData()
            if rtn >= 0:
                faOutput = faOutput + ("%0.6f,0,692,972,0,%0.3f,%0.3f\r\n" % (faData.Timestamp, faData.YForce[actuatorYIndex], faData.ZForce[actuatorZIndex]))
