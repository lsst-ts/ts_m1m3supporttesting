        ...
        for test in testTable:
            ...
            while (data.Timestamp - startTimestamp) < TEST_SAMPLE_TIME:
                ...
                if (data.Timestamp - startTimestamp) > TEST_HOLD_TIME and not commanded:
                    commanded = True
                    m1m3.ApplyOffsetForcesByMirrorForce(test[1], test[2], test[3], test[4], test[5], test[6], False)
                # Sample data
                rtn, data = m1m3.GetNextSampleHardpointActuatorData()
                if rtn >= 0:
                    output = output + ("%0.6f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f\r\n" % (data.Timestamp, data.MeasuredForce[0], data.MeasuredForce[1], data.MeasuredForce[2], data.MeasuredForce[3], data.MeasuredForce[4], data.MeasuredForce[5], data.Fx, data.Fy, data.Fz, data.Mx, data.My, data.Mz))
                    
            # Prepend force and timestamp information
            rtn, data = m1m3.GetEventAppliedOffsetForces()
            output = ("Applied %0.3f %0.3f %0.3f %0.3f %0.3f %0.3f at %0.6f\r\n" % (data.Fx, data.Fy, data.Fz, data.Mx, data.My, data.Mz, data.Timestamp)) + output
                       
            # Write the output file        
            path = GetFilePath("%d-ForceBalanceSystemCheck-%s.csv" % (int(startTimestamp), test[0]))
