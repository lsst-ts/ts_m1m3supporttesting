    ...
    def CheckHardpointActuators(self, m1m3, sim, state):
        ...
        for row in hardpointActuatorTable:
            for otherRow in hardpointActuatorTable:
                sim.setHPForceAndStatus(
                    otherRow[hardpointActuatorTableIDIndex], 0, 0, 0
                )
            ...
            for otherRow in hardpointActuatorTable:
                ...
                expected = 0
                if (
                    otherRow[hardpointActuatorTableIDIndex]
                    == row[hardpointActuatorTableIDIndex]
                ):
                    expected = 1
                Equal(
                    "HardpointActuatorData.Encoder[%d]" % pIndex,
                    data.Encoder[pIndex],
                    -expected,
                )
                InTolerance(
                    "HardpointActuatorData.MeasuredForce[%d]" % pIndex,
                    data.MeasuredForce[pIndex],
                    float(expected),
                    0.001,
                )
