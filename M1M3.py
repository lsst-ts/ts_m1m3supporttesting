...
class M1M3:
    ...
    def ApplyAberrationForcesByBendingModes(self, coefficients):
        Log(
            "M1M3: ApplyAberrationForcesByBendingModes([%s])"
        ...
        self.sal.waitForCompletion_ApplyAberrationForcesByBendingModes(
            cmdId, COMMAND_TIMEOUT
        )
        time.sleep(COMMAND_TIME)
    ...
    def ApplyActiveOpticForcesByBendingModes(self, coefficients):
        Log(
            "M1M3: ApplyActiveOpticForcesByBendingModes([%s])"
            % (",".join(map(str, coefficients)))
        )
        ...
        self.sal.waitForCompletion_ApplyActiveOpticForcesByBendingModes(
            cmdId, COMMAND_TIMEOUT
        )
        time.sleep(COMMAND_TIME)
    ...
    def ApplyOffsetForces(self, xForces, yForces, zForces, waitForCompletion=True):
        Log(
            "M1M3: ApplyOffsetForces([%s], [%s], [%s])"
            % (
                ",".join(map(str, xForces)),
                ",".join(map(str, yForces)),
                ",".join(map(str, zForces)),
            )
        )
    ...
    def ApplyOffsetForcesByMirrorForce(
        self, fx, fy, fz, mx, my, mz, waitForCompletion=True
    ):
        Log(
            "M1M3: ApplyOffsetForcesByMirrorForce(%s, %s, %s, %s, %s, %s)"
            % (fx, fy, fz, mx, my, mz)
        )
        ...
        if waitForCompletion:
            self.sal.waitForCompletion_ApplyOffsetForcesByMirrorForce(
                cmdId, COMMAND_TIMEOUT
            )
            time.sleep(COMMAND_TIME)
    ...
    def TranslateM1M3(
        ...
        Log(
            "M1M3: TranslateM1M3(%s, %s, %s, %s, %s, %s)"
            % (
                xTranslation,
                yTranslation,
                zTranslation,
    ...
    def ModbusTransmit(self, actuatorId, functionCode, rawData):
        Log(
            "M1M3: ModbusTransmit(%d, %d, %d)"
            % (actuatorId, functionCode, len(rawData))
        )
    ...
    def Modbus_SetADCOffsetSensitivity(self, actuatorId, channel, offset, sensitivity):
        Log(
            "M1M3: Modbus SetADCOffsetSensitivity(%d, %d, %f, %f)"
            % (actuatorId, channel, offset, sensitivity)
        )
