...
class CellSimulator:
    ...
    def __init__(self, ipAddress, prnt=True, ignore=True):
        ...
        if not ignore:
            ...
            self._udpResponse = UDP.UDP(
                socket.gethostbyname(socket.gethostname()), 4999, True
            )
    ...
    def setDisplacement(
        ...
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting IMS displacements to (%0.3f, %0.3f, %0.3f, %0.3f, %0.3f, %0.3f, %0.3f, %0.3f)"
                    % (d1, d2, d3, d4, d5, d6, d7, d8)
                )
            self._udpClientDisplace.send(
                self._displaceSim.displacementResponse(d1, d2, d3, d4, d5, d6, d7, d8)
            )
    ...
    def setAccelerometerVoltage(
        ...
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting accelerometer voltages to (%0.3f, %0.3f, %0.3f, %0.3f, %0.3f, %0.3f, %0.3f, %0.3f)"
                    % (a1, a2, a3, a4, a5, a6, a7, a8)
                )
            self._udpClientAccel.send(
                self._accelSim.accelerometerResponse(
                    accelerometerNumber=1, elevationVoltage=a1, azimuthVoltage=a2
                )
            )
            self._udpClientAccel.send(
                self._accelSim.accelerometerResponse(
                    accelerometerNumber=2, elevationVoltage=a3, azimuthVoltage=a4
                )
            )
            self._udpClientAccel.send(
                self._accelSim.accelerometerResponse(
                    accelerometerNumber=3, elevationVoltage=a5, azimuthVoltage=a6
                )
            )
            self._udpClientAccel.send(
                self._accelSim.accelerometerResponse(
                    accelerometerNumber=4, elevationVoltage=a7, azimuthVoltage=a8
                )
            )
    ...
    def setAccelerometer(
        ...
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting accelerometer acceleration to (%0.3f, %0.3f, %0.3f, %0.3f, %0.3f, %0.3f, %0.3f, %0.3f)"
                    % (a1, a2, a3, a4, a5, a6, a7, a8)
                )
    ...
    def setAngularAcceleration(self, ax: float, ay: float, az: float):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting accelerometer angular acceleration to (%0.3f, %0.3f, %0.3f)"
                    % (ax, ay, az)
                )
    ...
    def setAngularVelocity(
        self, vx: float, vy: float, vz: float, valid: bool, temperature: int
    ):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting gyro angular velocity to (%0.3f, %0.3f, %0.3f, %s, %d)"
                    % (vx, vy, vz, valid, temperature)
                )
            self._udpClientGyro.send(
                self._gyroSim.loadData(vx, vy, vz, valid, temperature)
            )
    ...
    def setAUXPowerNetworksOff(self, off):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting AUX power network off to (%d)"
                    % (self.boolToInt(off))
                )
            self._udpClientDI.send(
                self._diSim.powerNetworkShutDown(self.boolToInt(not off))
            )
    ...
    def setThermalEquipmentOff(self, off):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting thermal equipment off to (%d)"
                    % (self.boolToInt(off))
                )
            self._udpClientDI.send(
                self._diSim.fansHeatersPumpPoweredOff(self.boolToInt(not off))
            )
    ...
    def setAirSupplyOff(self, off):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting air supply off to (%d)"
                    % (self.boolToInt(off))
                )
            self._udpClientDI.send(
                self._diSim.airSupplyClosedAirReliefOpen(self.boolToInt(not off))
            )
    ...
    def setCabinetDoorOpen(self, open):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting cabinet door open to (%d)"
                    % (self.boolToInt(open))
                )
            self._udpClientDI.send(
                self._diSim.gisEarthquakeSignal(self.boolToInt(not open))
            )
    ...
    def setTMAMotionStop(self, stop):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting TMA motion stop to (%d)"
                    % (self.boolToInt(stop))
                )
    ...
    def setGISHeartbeatLost(self, lost):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting GIS heartbeat lost to (%d)"
                    % (self.boolToInt(lost))
                )
            self._udpClientDI.send(
                self._diSim.gisHeartbeatLost(self.boolToInt(not lost))
            )
    ...
    def setAirSupplyValveOpen(self, open):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting air supply valve open to (%d)"
                    % (self.boolToInt(open))
                )
            self._udpClientDI.send(
                self._diSim.airSupplyValveStatusOpen(self.boolToInt(not open))
            )
    ...
    def setAirSupplyValveClosed(self, closed):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting air supply valve closed to (%d)"
                    % (self.boolToInt(closed))
                )
            self._udpClientDI.send(
                self._diSim.airSupplyValveStatusClosed(self.boolToInt(not closed))
            )
    ...
    def setILCID(
        ...
        ilcAppType: int,
        networkNodeType: int,
        ilcSelectedOptions: int,
        networkNodeOptions: int,
        majorRev: int,
        minorRev: int,
        firmwareName: str,
    ):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting ILC ID for %d to (%d, %d, %d, %d, %d, %d, %d, %s)"
                    % (
                        id,
                        uniqueId,
                        ilcAppType,
    ...
    def setILCStatus(self, id: int, mode: int, status: int, faults: int):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting ILC status for %d to (%d, %d, %d)"
                    % (id, mode, status, faults)
                )
            subnet, address = self.getSubnetAndAddress(id)
    ...
    def setHPForceAndStatus(
        self, id: int, statusByte: int, ssiEncoderValue: int, loadCellForce: float
    ):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting HP force and status for %d to (%d, %d, %0.3f)"
                    % (id, statusByte, ssiEncoderValue, loadCellForce)
                )
            subnet, address = self.getSubnetAndAddress(id)
            subnet.send(
                self._ilcSim.forceAndStatusRequest(
                    address, statusByte, ssiEncoderValue, float(loadCellForce)
                )
            )

    def setBoostValveGains(
        self, id: int, primaryCylinderGain: float, secondaryCylinderGain: float
    ):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting boost valve gains for %d to (%0.3f, %0.3f)"
                    % (id, primaryCylinderGain, secondaryCylinderGain)
                )
            subnet, address = self.getSubnetAndAddress(id)
            subnet.send(
                self._ilcSim.readBoostValueDcaGains(
                    address, float(primaryCylinderGain), float(secondaryCylinderGain)
                )
            )

    def setFAForceAndStatus(
        self,
        id: int,
        statusByte: int,
        primaryCylinderForce: float,
        secondaryCylinderForce: float = 0,
    ):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting FA force and status for %d to (%d, %0.3f, %0.3f)"
                    % (id, statusByte, primaryCylinderForce, secondaryCylinderForce)
                )
            subnet, address = self.getSubnetAndAddress(id)
            if address <= 16:
                subnet.send(
                    self._ilcSim.singlePneumaticAxisForce(
                        address, statusByte, float(primaryCylinderForce)
                    )
                )
                subnet.send(
                    self._ilcSim.singlePneumaticForceAndStatus(
                        address, statusByte, float(primaryCylinderForce)
                    )
                )
            else:
                subnet.send(
                    self._ilcSim.dualPneumaticAxisForce(
                        address,
                        statusByte,
                        float(primaryCylinderForce),
                        float(secondaryCylinderForce),
                    )
                )
                subnet.send(
                    self._ilcSim.dualPneumaticForceAndStatus(
                        address,
                        statusByte,
                        float(primaryCylinderForce),
                        float(secondaryCylinderForce),
    ...
    def setADCSampleRate(self, id: int, scanRateCode: int):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting ADC sample rate for %d to (%d)"
                    % (id, scanRateCode)
                )
            subnet, address = self.getSubnetAndAddress(id)
    ...
    def setCalibrationData(
        self,
        id: int,
        mainAdcCalibration1: float,
        mainAdcCalibration2: float,
        mainAdcCalibration3: float,
        mainAdcCalibration4: float,
        mainSensorOffset1: float,
        mainSensorOffset2: float,
        mainSensorOffset3: float,
        mainSensorOffset4: float,
        mainSensorSensitivity1: float,
        mainSensorSensitivity2: float,
        mainSensorSensitivity3: float,
        mainSensorSensitivity4: float,
        backupAdcCalibration1: float,
        backupAdcCalibration2: float,
        backupAdcCalibration3: float,
        backupAdcCalibration4: float,
        backupSensorOffset1: float,
        backupSensorOffset2: float,
        backupSensorOffset3: float,
        backupSensorOffset4: float,
        backupSensorSensitivity1: float,
        backupSensorSensitivity2: float,
        backupSensorSensitivity3: float,
        backupSensorSensitivity4: float,
    ):
        if not self.Ignore:
            if self.Print:
            ...
            subnet, address = self.getSubnetAndAddress(id)
            subnet.send(
                self._ilcSim.readCalibrationData(
                    address,
                    mainAdcCalibration1,
                    mainAdcCalibration2,
                    mainAdcCalibration3,
                    ...
                    mainSensorOffset3,
                    mainSensorOffset4,
                    mainSensorSensitivity1,
                    mainSensorSensitivity2,
                    mainSensorSensitivity3,
                    mainSensorSensitivity4,
                    backupAdcCalibration1,
                    backupAdcCalibration2,
                    backupAdcCalibration3,
                    backupAdcCalibration4,
                    backupSensorOffset1,
                    backupSensorOffset2,
                    backupSensorOffset3,
                    backupSensorOffset4,
                    backupSensorSensitivity1,
                    backupSensorSensitivity2,
                    backupSensorSensitivity3,
                    backupSensorSensitivity4,
                )
            )
    ...
    def setPressure(self, id: int, p1: float, p2: float, p3: float, p4: float):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting pressure for %d to (%0.3f, %0.3f, %0.3f, %0.3f)"
                    % (id, p1, p2, p3, p4)
                )
            subnet, address = self.getSubnetAndAddress(id)
    ...
    def setMezzanineID(
        self, id: int, uniqueId: int, firmwareType: int, firmwareVersion: int
    ):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting mezzanine ID for %d to (%d, %d, %d)"
                    % (id, uniqueId, firmwareType, firmwareVersion)
                )
            subnet, address = self.getSubnetAndAddress(id)
            subnet.send(
                self._ilcSim.reportDcaId(
                    address, uniqueId, firmwareType, firmwareVersion
                )
            )
    ...
    def setMezzanineStatus(self, id: int, status: int):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting mezzanine status for %d to (%d)"
                    % (id, status)
                )
            subnet, address = self.getSubnetAndAddress(id)
    ...
    def setLVDT(self, id: int, lvdt1: float, lvdt2: float):
        if not self.Ignore:
            if self.Print:
                Log(
                    "CellSimulator: Setting LVDT for %d to (%0.3f, %0.3f)"
                    % (id, lvdt1, lvdt2)
                )
            subnet, address = self.getSubnetAndAddress(id)
    ...
    def getSubnetAndAddress(self, id: int):
        for row in hardpointActuatorTable:
            if row[hardpointActuatorTableIDIndex] == id:
                return (
                    self.getSubnet(row[hardpointActuatorTableSubnetIndex]),
                    row[hardpointActuatorTableAddressIndex],
                )
        for row in hardpointMonitorTable:
            if row[hardpointMonitorTableIDIndex] == id:
                return (
                    self.getSubnet(row[hardpointMonitorTableSubnetIndex]),
                    row[hardpointMonitorTableAddressIndex],
                )
        for row in forceActuatorTable:
            if row[forceActuatorTableIDIndex] == id:
                return (
                    self.getSubnet(row[forceActuatorTableSubnetIndex]),
                    row[forceActuatorTableAddressIndex],
                )
