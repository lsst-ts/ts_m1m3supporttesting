    ...
    def CheckForceActuators(self, m1m3, sim, state):
        ...
        for row in forceActuatorTable:
            ...
            for otherRow in forceActuatorTable:
                ...
                expected = 0.0
                if (
                    otherRow[forceActuatorTableIDIndex]
                    == row[forceActuatorTableIDIndex]
                ):
                    expected = 1.0
                InTolerance(
                    "ForceActuatorData.PrimaryCylinderForce[%d]" % pIndex,
                    data.PrimaryCylinderForce[pIndex],
                    expected,
                    0.001,
                )
                if sIndex != -1:
                    InTolerance(
                        "ForceActuatorData.SecondaryCylinderForce[%d]" % sIndex,
                        data.SecondaryCylinderForce[sIndex],
                        -expected,
                        0.001,
                    )
