...
class DigitalInputSimulator(Simulator):
    ...
    def powerNetworkShutDown(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the Power Network Shut Down can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
    ...
    def fansHeatersPumpPoweredOff(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the Fan Coils/Heaters/Pump Powered Off can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
    ...
    def laserTrackerOff(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the Laser Tracker Off can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
    ...
    def airSupplyClosedAirReliefOpen(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the Air Supply Closed can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
    ...
    def gisEarthquakeSignal(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the GIS Earthquake Signal can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
    ...
    def gisEStop(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the GIS E Stop can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
    ...
    def tmaMotionStop(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the TMA Motion Stop can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
    ...
    def gisHeartbeatLost(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the GIS Heartbeat Lost can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
    ...
    def airSupplyValveStatusOpen(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the Air Supply Valve Status Open can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
    ...
    def airSupplyValveStatusClosed(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the Air Supply Valve Status Closed can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
    ...
    def mirrorCellLightsOn(self, command: int):
        if command != 0 and command != 1:
            raise Exception(
                "The command for the Mirror Cell Lights On can only be 0 (false) or 1 (true).  The value is currently"
                + str(command)
            )
        response = bytearray()
...
def main():
    ...
    print(
        "Digital Input, Power Network Shut Down Command: "
        + str(binascii.hexlify(response))
    )

    response = DISim.fansHeatersPumpPoweredOff(1)
    assert bytes([2, 1, 1]) == response
    print(
        "Digital Input, Fan Coils/Heaters/Pump Powered Off Command: "
        + str(binascii.hexlify(response))
    )
    ...
    print(
        "Digital Input, Laser Tracker Off Command: " + str(binascii.hexlify(response))
    )

    response = DISim.airSupplyClosedAirReliefOpen(1)
    assert bytes([2, 3, 1]) == response
    print(
        "Digital Input, Air Supply Closed Command: " + str(binascii.hexlify(response))
    )

    response = DISim.gisEarthquakeSignal(1)
    assert bytes([2, 4, 1]) == response
    print(
        "Digital Input, GIS Earthquake Signal Command: "
        + str(binascii.hexlify(response))
    )
    ...
    print(
        "Digital Input, GIS Heartbeat Lost Command: " + str(binascii.hexlify(response))
    )

    response = DISim.airSupplyValveStatusOpen(1)
    assert bytes([2, 8, 1]) == response
    print(
        "Digital Input, Air Supply Valve Status Open Command: "
        + str(binascii.hexlify(response))
    )
    ...
    print(
        "Digital Input, Air Supply Valve Status Closed Command: "
        + str(binascii.hexlify(response))
    )
    ...
    print(
        "Digital Input, Mirror Cell Lights On Command: "
        + str(binascii.hexlify(response))
    )
